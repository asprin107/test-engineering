...
resource "aws_subnet" "subnets_public" {
  ...
  tags = merge(
    {
      Name = "${var.project}-pub-sbn-${var.rule.subnets.available_zones[count.index % length(var.rule.subnets.available_zones)]}"
    }
  )
}
...
resource "aws_subnet" "subnets_private" {
  ...
  tags = merge(
    {
      Name = "${var.project}-prv-sbn-${var.rule.subnets.available_zones[count.index % length(var.rule.subnets.available_zones)]}"
    }
  )
}